...
3. Restart, then in Cowork as an admin: `bridge sync` → run
   **systemprompt_setup_admin** → it reports your artifact as *stale* → accept
   the replacement. The skill matches by id only and verifies each installed
   allowlist, repairing mismatches itself.
...
`/systemprompt_setup_admin` executing in Cowork — the only skill that installs
dashboards, and admin-only. The agent reads the bundled manifest, stages the
HTML, calls `create_artifact` once per record, and verifies the install: all
eleven appear under *Pinned*, including the four workspace dashboards
(`todo-bulletin`, `upcoming-deals`, `pipeline-open-deals`, `recent-activity`,
from the `systemprompt-workspace` bundle). The behaviour is **prose, not
code**: `services/skills/systemprompt_setup_admin/SKILL.md`. Edit the SKILL.md
to change how the agent installs or verifies; the bundle it installs is
generated (section 4). Start a new setup-style skill by copying this
directory — section 6c step 4.
