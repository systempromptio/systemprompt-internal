...
use systemprompt_web_site::skills_page::{SkillEntry, SkillsPageConfig};
...
",
    ...
    .expect("minimal skill entry");

    assert!(enabled.enabled);
    assert!(enabled.tags.is_empty());
    assert!(enabled.category.is_none());
    assert!(enabled.display_category.is_none());
    assert!(enabled.href.is_none());

    let disabled: SkillEntry = serde_yaml::from_str(
        r"
id: retired
name: Retired
...
",
    ...
    .expect("disabled skill entry");
    assert!(!disabled.enabled);
}

#[test]
fn a_skill_entry_without_an_id_name_or_description_is_rejected() {
    for yaml in [
        "name: No Id\ndescription: d\n",
        "id: no_name\ndescription: d\n",
        "id: no_desc\nname: No Desc\n",
        ...
        assert!(
            serde_yaml::from_str::<SkillEntry>(yaml).is_err(),
            "{yaml:?} must not deserialise"
        );
    }
}

#[test]
fn the_skills_page_config_round_trips_through_yaml() {
    let config: SkillsPageConfig = serde_yaml::from_str(
        r"
skills:
  - id: a
    name: A
...
",
    ...
    .expect("skills page config");

    let round_tripped: SkillsPageConfig =
        serde_yaml::from_str(&serde_yaml::to_string(&config).expect("serialises"))
            .expect("re-reads its own output");
