...
fn list_mcp_servers_skips_a_file_that_is_not_valid_yaml() {
    ...
    assert_eq!(servers[0].id.as_str(), "odoo");
}
...
fn list_mcp_servers_reads_every_declared_field() {
  ...
  odoo:
    type: remote
    binary: odoo-mcp
    package: '@systemprompt/odoo-mcp'
    port: 7100
    endpoint: https://sf.example.test/mcp
    description: Odoo accessor
    enabled: false
    oauth:
      required: true
      scopes:
...
",
    ...
    assert_eq!(server.package_name, "@systemprompt/odoo-mcp");
    assert_eq!(server.port, 7100);
    assert_eq!(server.description, "Odoo accessor");
...
fn list_mcp_servers_records_the_source_path_relative_to_the_services_root() {
    ...
    assert_eq!(servers[0].source_path, "services/mcp/odoo.yaml");
}
