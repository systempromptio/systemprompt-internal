...
//!
//! Fixtures create departments through raw SQL because `departments.org_id` is
//! NOT NULL from migration 022 and the repository's own `create_department`
//! does not set it — see `create_department_cannot_satisfy_the_org_id_column`.

use systemprompt_web_admin::repositories::departments;
use systemprompt_web_admin::types::DepartmentInput;

use crate::fixtures::{
    insert_acl_rule, insert_department, insert_user, set_department, unclaimed_email, unique,
};
...
async fn find_department_reads_back_what_was_inserted() {
    ...
    insert_department(&db.pool, &id, &name, HOUSE).await;
...
async fn update_department_rewrites_name_and_description() {
    ...
    let id = unique("dept");
    insert_department(&db.pool, &id, &unique("Before"), HOUSE).await;
...
async fn update_department_carries_members_across_a_rename() {
    ...
    insert_department(&db.pool, &id, &before, HOUSE).await;
