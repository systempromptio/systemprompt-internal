...
use crate::fixtures::{insert_acl_rule, unique};
...
async fn list_rules_for_entity_returns_only_that_entitys_grants() {
    ...
    insert_acl_rule(&db.pool, "skill", &wanted, "user", "alice", "allow").await;
    insert_acl_rule(&db.pool, "skill", &other, "user", "bob", "allow").await;
...
async fn list_rules_for_entity_orders_by_rule_type_then_value() {
    ...
    insert_acl_rule(&db.pool, "skill", &entity, "user", "zoe", "allow").await;
    insert_acl_rule(&db.pool, "skill", &entity, "role", "admin", "allow").await;
    insert_acl_rule(&db.pool, "skill", &entity, "user", "adam", "allow").await;
...
async fn list_all_rules_includes_every_entity_type() {
    ...
    insert_acl_rule(&db.pool, "skill", &skill, "user", "alice", "allow").await;
    insert_acl_rule(&db.pool, "mcp_server", &server, "role", "admin", "deny").await;
...
#[tokio::test]
async fn count_assignments_by_entity_type_counts_grants_per_entity() {
    let Some(db) = TempDb::create().await else {
        return;
    };
    let busy = unique("skill");
    let quiet = unique("skill");
    insert_acl_rule(&db.pool, "skill", &busy, "user", "alice", "allow").await;
    insert_acl_rule(&db.pool, "skill", &busy, "user", "bob", "allow").await;
    insert_acl_rule(&db.pool, "skill", &quiet, "role", "admin", "allow").await;
    insert_acl_rule(
        &db.pool,
        "mcp_server",
        &unique("server"),
    ...
    let counts = count_assignments_by_entity_type(&db.pool, "skill")
        .await
        .expect("count assignments");

    assert_eq!(counts.get(&busy), Some(&2));
    assert_eq!(counts.get(&quiet), Some(&1));
    db.cleanup().await;
}

#[tokio::test]
async fn count_assignments_by_entity_type_is_empty_for_an_unused_type() {
    let Some(db) = TempDb::create().await else {
        return;
    };

    let counts = count_assignments_by_entity_type(&db.pool, "teams_conversation")
        .await
        .expect("count assignments");
...
async fn bulk_set_rules_replaces_grants_that_were_already_there() {
    ...
    let entity = unique("skill");
    insert_acl_rule(&db.pool, "skill", &entity, "user", "stale", "allow").await;
