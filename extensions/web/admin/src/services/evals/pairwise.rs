...
pub(crate) async fn execute_pairwise(params: PairwiseParams<'_>) -> Result<RunTally, sqlx::Error> {
    ...
    for case in params.cases {
        compare_one(&params, case, &mut tally).await?;
    }
    Ok(tally)
}

// Why: one case, answered by both models, then judged in both orders.
async fn compare_one(
    params: &PairwiseParams<'_>,
    case: &EvalCaseRow,
    tally: &mut RunTally,
) -> Result<(), sqlx::Error> {
    ...
    let (answer_a, answer_b) = tokio::join!(
        replay::answer_for(params.pool, params.config, params.model_a, &prompt),
        replay::answer_for(params.pool, params.config, params.model_b, &prompt),
    );
    ...
    let decide = |w: PairWinner| if agreed { w } else { PairWinner::Tie };

    for (winner, swapped, rationale) in [
        (
            decide(forward_winner),
            false,
            forward.verdict.rationale.as_str(),
        ),
        (
            decide(reverse_winner),
            true,
