...
use crate::repositories::dashboard::{session_registry, usage_aggregations};
...
pub(crate) async fn track_statusline_event(
    ...
    if let Some(session_id) = query.session_id.as_ref() {
        ...
        let context_pct = window.and_then(|window| {
            let size = window.context_window_size.filter(|size| *size > 0)?;
            let usage = window.current_usage?;
            let used = usage
                .input
                .unwrap_or(0)
                .saturating_add(usage.output.unwrap_or(0));
            i16::try_from((used.saturating_mul(100) / size).clamp(0, 100)).ok()
        });
        session_registry::update_session_statusline(&session_registry::StatuslineParams {
            pool: &pool,
            session_id,
            model: model.as_deref(),
            live_cost_microdollars: cost,
            context_pct,
        })
        .await;
