...
fn build_raw_key(user_id: &UserId, session_id: &SessionId, payload: &HookEventPayload) -> String {
    ...
    match &payload.event {
        ...
        HookEvent::PostToolUse(d) => post_tool_use_key(uid, session, d),
        HookEvent::PostToolUseFailure(d) => post_tool_failure_key(uid, session, d),
        ...
        HookEvent::TaskCompleted(d) => format!("{uid}:{session}:TaskCompleted:{}", d.task_id),
