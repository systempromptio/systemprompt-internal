...
fn validate(doc: &PlansDoc) -> Result<ValidatedDoc<'_>, MarketplaceError> {
    ...
    for org in &doc.organizations {
        ...
        for grant in &plan.grants {
            let kind = EntityKind::from_str(&grant.entity_type).map_err(|e| {
                MarketplaceError::Internal(format!("{PLANS_FILE}: {}: {e}", org.slug))
            })?;
            let access = Access::from_str(&grant.access).map_err(|e| {
                MarketplaceError::Internal(format!("{PLANS_FILE}: {}: {e}", org.slug))
            })?;
