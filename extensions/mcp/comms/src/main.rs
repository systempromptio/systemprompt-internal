//! Entry point for the `comms` MCP server binary.
...
use systemprompt_mcp_comms::CommsServer;
use tokio::net::TcpListener;

const DEFAULT_SERVICE_ID: &str = "comms";
...
async fn main() -> Result<()> {
    ...
    // first.
    systemprompt_mcp_comms::schema::ensure_installed(ctx.db_pool())
        .await
        .context("Failed to install the comms schema")?;

    let server = CommsServer::new(
        Arc::clone(ctx.db_pool()),
        service_id.clone(),
        Arc::clone(ctx.authz_hook()),
    )
    .context("Failed to initialize CommsServer")?;
