...
impl Assets {
    ...
    pub fn font_face_css(&self) -> FactsheetResult<String> {
        ...
        let archivo = font_data_uri(
            &fonts.join("ArchivoBlack-Regular.ttf"),
            FontFormat::TrueType,
        )?;
    ...
    // Why: Read an SVG and strip the XML declaration and doctype, which are illegal
    ...
    pub fn logo_on_dark(&self) -> FactsheetResult<String> {
        Ok(self
            .logo_on_light()?
            .replace(&format!("fill=\"{LOGO_INK}\""), "fill=\"#FFFFFF\""))
    }

    pub fn brand_mark(&self, name: &str) -> FactsheetResult<String> {
        let svg = self.load_svg(name)?;
        // Why: Claude ships a white wordmark; the others carry no fill at all and
...
// Why: Force explicit dimensions on the root `<svg>`, dropping any it already
