...
4. **Check terminology and brand usage.** Company name in prose is "systemprompt.io" (lowercase, always with the `.io`; never "Systemprompt Inc.", never "SP"); the product is "Systemprompt Internal"; the URL form is systemprompt.io; support contact is ed@tyingshoelaces.com. Odoo is the system of record - say "Odoo" for ERP/CRM data (leads are `crm.lead`, notes are `mail.message`, activities are `mail.activity`), and "Systemprompt Internal" for the AI and communication layer on top of it. Never describe the two as one system. It is a library you embed and own, never a "framework".
